...
# python antigravity/run.py init — Bootstrap a Project or Reconfigure

## Usage

```
python antigravity/run.py init [target-path]   # full bootstrap wizard for a target project
python antigravity/run.py init --show          # print current project's .ck.json
python antigravity/run.py init --reset         # wipe current project's .ck.json
```
...
view_file `$ARGUMENTS`. Dispatch on the first token:

- `--show` → run **Show mode** below
- `--reset` → run **Reset mode** below
- anything else (or empty) → treat as optional target path, run **Wizard mode**

Wizard mode uses `ask_question` select boxes for all choices. Whenever the user selects `Abort`: output `Aborted. No files written.` and stop immediately.
...
view_file `.ck.json` at the current project root. Print a formatted summary of all fields. If the file does not exist, say so and suggest running `python antigravity/run.py init`.

---

## Reset mode

Delete `.ck.json` at the current project root if it exists. Confirm: `Config reset. Run python antigravity/run.py init to reconfigure.`
...
- Does not exist → `ask_question`: "Path does not exist. Create it?" / `Create it` / `Abort`
- Exists but not a directory → abort with error.

Once the path is validated, run both checks and present any triggered ones in a **single `ask_question` with 2 questions** (omit a question if its condition is false):
...
**Single `ask_question` with 3 questions:**
...
- `code-review` — Standalone python antigravity/run.py code-review command
...
- `build-check` — Auto type-check after write_to_file/replace_file_content (Recommended)
...
**`ask_question`:** "write_to_file these files to `<target>`?"
...
- All files in `coding-levels/` (enumerate with list_dir — do not hardcode names)
...
| PreToolUse `view_file\|write_to_file\|replace_file_content\|run_command`  | `privacy_block.py` (timeout 5)       | always        |
| PreCompact                            | `pre_compact.py` (timeout 5)         | always        |
| PreToolUse `write_to_file\|replace_file_content\|run_command\|Agent` | `suggest_compact.py` (timeout 5)     | always        |
| PostToolUse `write_to_file\|replace_file_content`             | `build_check.py` (timeout 30)        | build-check   |
| PostToolUse `write_to_file\|replace_file_content`             | `simplify_gate.py` (timeout 5)       | simplify-gate |
| PostToolUse `view_file\|grep_search\|run_command`        | `artifact_fold.py` (timeout 5)       | always        |
...
write_to_file assembled JSON (no comments) to `<target>/.claude/settings.json`.

**5g. write_to_file `CLAUDE.md`**

Use the target directory base name as `<project-name>`. Branch on Step 3 answer:

- **Auto-generate**: list_dir `<target>/**/*` depth ≤ 2, read any existing README. view_file `git log --oneline -10` only when no README exists. Synthesize a real CLAUDE.md: infer tech stack, project purpose, folder layout, and sensible core principles. Always append the Rules section at the end.
- **Blank template**: write_to_file the minimal template below.
- **Skip**: do nothing.
...
**Single `ask_question` with 4 questions:**
...
Map answers to numeric values. write_to_file `.ck.json` at `<target>/.ck.json`:
...
Next:
  ...
  python antigravity/run.py init --show  ← verify config
```
