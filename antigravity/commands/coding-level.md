...
```
python antigravity/run.py coding-level          # show current level + interactive menu
python antigravity/run.py coding-level -1       # use Claude's default style (no injection)
python antigravity/run.py coding-level 3        # set to Senior
python antigravity/run.py coding-level reset    # remove config entirely (same as -1 but clears the key)
```

## How to execute this command

view_file the argument `$ARGUMENTS`:
