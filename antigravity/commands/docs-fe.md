...
view_file the resolved files to list every endpoint added or modified:
- HTTP method + path
...
### 6. write_to_file the output file
...
write_to_file a clean, self-contained HTML file that expresses the same content as the MD template above. No fixed structure required — choose whatever layout best presents the data. Requirements:
