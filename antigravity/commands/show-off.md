...
# python antigravity/run.py show-off — HTML Presentation + Social Capture

## Usage

```
python antigravity/run.py show-off [--auto | --fast | --clone] <topic>
```
...
- **`--clone`** — Re-capture existing HTML in `tmppython antigravity/run.py show-off-<slug>/`, skip generation
...
Output dir: `<project-root>/tmppython antigravity/run.py show-off-<slug>/`
...
Example: "neon, 1x1, dark, en / Fastest way to ship · python antigravity/run.py plan, python antigravity/run.py cook, python antigravity/run.py fix · pipeline / auto"
```
...
`auto` background: search for a royalty-free photo via Unsplash API or pick randomly from `tmppython antigravity/run.py show-off-<slug>/backgrounds/` if it exists.
...
write_to_file `tmppython antigravity/run.py show-off-<slug>/index.html` — fully self-contained, no build step.
...
```
📄 Preview ready: tmppython antigravity/run.py show-off-<slug>/index.html  Style: <style>
...
```
HTML_PATH   = <project-root>/tmppython antigravity/run.py show-off-<slug>/index.html
OUTPUT_DIR  = <project-root>/tmppython antigravity/run.py show-off-<slug>/images
...
```
✅ Show-off ready: tmppython antigravity/run.py show-off-<slug>/
   Style: <style> · <viewports> · <theme> · <lang>
   Preview : tmppython antigravity/run.py show-off-<slug>/index.html
   Images  : tmppython antigravity/run.py show-off-<slug>/images/  (<N> files)
```
...
- `python antigravity/run.py show-off --clone` — re-capture after manual HTML edits
- `python antigravity/run.py show-off --auto <topic>` — zero-prompt batch generation
