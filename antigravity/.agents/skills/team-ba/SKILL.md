...
- **`--context "{text or path}"`** — extra context. If the value starts with `./` or `/`, use the view_file tool to read it as a file. Otherwise treat as inline text. Prepend to your analysis; do NOT write it to any artifact file.
...
Use the view_file tool: `projects/{slug}/team/.project-config.md`
...
## Step 4 — write_to_file Artifact Files

write_to_file all 4 files completely. Do NOT use placeholders. write_to_file each file in full before starting the next.
...
| ----- | ---- | --------------------------------------------- | --------------------------- | -------------------- |
| ...   | ...  | Non-technical / Basic / Intermediate / Expert | Daily / Weekly / Occasional | view_file / write_to_file / Admin |
...
After writing all 4 files, validate structural completeness. Use the view_file tool to re-read each file.
...
- **Attempt 3:** HARD STOP. write_to_file failure log:
