...
**write_to_file-Through** — keeps cache in sync on every write, at the cost of write latency:
...
**write_to_file-Behind (write_to_file-Back)** — write to cache only, flush to DB asynchronously. Fastest writes; risk of data loss on crash. Use only when write throughput is extreme and some loss is tolerable.
