...
# at:srs — IEEE 830 SRS from Raw Requirements
...
Ask these 3 questions in one `ask_question` batch:
...
view_file full input silently. Emit:
```
...
If P1 gaps exist, emit `ask_question` with ≤7 questions. Label:
```
...
If P2 gaps remain, emit `ask_question` with ≤7 questions. Label:
```
...
Emit `ask_question` with ≤7 questions. Label:
```
