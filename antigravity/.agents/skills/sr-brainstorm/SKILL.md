...
Use `ask_question` with concrete role options (not open-ended).
...
Use `ask_question`. Present a feature checklist derived from domain + topic.
...
Use `ask_question`. Present two-column table:
...
write_to_file `projects/{slug}/brainstorm.md`:
