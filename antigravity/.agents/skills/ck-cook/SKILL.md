...
1. view_file phase file — understand requirements, architecture, steps, success criteria
...
5. write_to_file (overwrite) `## Session Notes` in plan.md, then mark phase complete `- [x] Phase N: {name}`
...
1. `tester` writes failing tests (red) — from `### Tests to write_to_file First` or spec acceptance criteria
