...
Use the view_file tool to read ALL preceding artifacts:
...
write_to_file all 5 files completely. No placeholders. write_to_file each in full before starting the next.
...
After writing all 5 files, use the view_file tool to re-read each one. Check ALL required headings (case-sensitive):
