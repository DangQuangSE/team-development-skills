...
metadata:
  ...
  do_not_use_when: User only wants a quick SRS from existing requirements (use /at:srs instead)
  hard_rules: >
...
Each round: ask ONE category of questions using `ask_question`.
...
write_to_file `projects/{slug}/spec.md` using the spec template structure.

**No word limit.** write_to_file every section fully — do not summarize or truncate.
...
Present the following menu using `ask_question`:

```
Spec is ready. What would you like to do next?

A) write_to_file plan          → break SRS into per-section plan files, then review
...
**For each file:**
- write_to_file the full planned content for that SRS section
...
Present summary table from Phase 4. Then ask via `ask_question`:
...
write_to_file `projects/{slug}/improvement-report.md`:
...
write_to_file `projects/{slug}/_context/`:
