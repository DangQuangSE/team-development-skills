...
Use the view_file tool to read ALL BA and TechLead artifacts:
...
8. Use list_dir tool: `projects/{slug}/team/techlead/ADR-*.md` → read all ADR files found
...
After writing all 3 files, use the view_file tool to re-read each one. Check ALL required headings (case-sensitive):
...
[
  ...
  { "content": "write_to_file unit tests for auth service", "status": "pending", "activeForm": "Writing unit tests for auth service" }
]
```
