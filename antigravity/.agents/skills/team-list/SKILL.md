...
Use the list_dir tool to find all project directories:
...
Use the list_dir tool to check each path. If the key file exists → phase is ✓ complete. If not → phase is ○ not started.

Also check:
- `projects/{slug}/validation-errors/` — if any `.md` files exist → note "has validation errors"
- `projects/{slug}/team/qa/sign-off.md` — if exists, use view_file tool to extract the Verdict line
