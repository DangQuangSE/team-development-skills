...
Use the view_file tool to read ALL BA artifacts:
...
write_to_file all files completely. No placeholders. write_to_file each file in full before starting the next.
...
After writing all files, use the view_file tool to re-read each one. Check ALL required headings (case-sensitive):
